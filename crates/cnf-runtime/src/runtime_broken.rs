use crate::control_flow::{
    ...
    LoopContext, ScopeManager,
};
use crate::dag::Dag;
use crate::scheduler::CallStack;
use serde_json_main as serde_json;
use serde_main as serde;
use cobol_protocol_v154::{compress_csm, CsmDictionary};
use cnf_compiler::Instruction;
use cnf_governance::{
    PolicyEngine, SovereigntyChecker, AccessControl, 
    audit_authority::AuditLedger, ExecutionTrace,
};
use std::collections::HashMap;

/// Error type for CENTRA-NF runtime operations
///
/// Designed for fail-fast semantics: all errors must be explicit,
/// noisy, and include context. The error must answer:
/// - What failed? (InvalidInstruction, EncryptionFailed, etc.)
/// - Why did it fail? (message: String)
///
/// This type is PUBLIC and exported from lib.rs so downstream
/// crates can construct Result<T, CnfError> types.
#[derive(Debug, Clone)]
pub enum CnfError {
    // === Buffer Management ===
    /// Buffer name not found in buffer registry.
    BufferNotFound(String),

    // === Instruction Dispatch ===
    /// Instruction is syntactically invalid or unsupported.
    /// Examples: unknown operation, wrong parameter count, malformed arguments.
    InvalidInstruction(String),

    // === Cryptography ===
    /// Encryption operation failed (e.g., key derivation, cipher mode error).
    EncryptionFailed(String),
    /// Decryption operation failed (e.g., authentication tag mismatch).
    DecryptionFailed(String),

    // === I/O ===
    /// I/O operation failed (e.g., file read, network send).
    IoError(String),

    // === Runtime ===
    /// General runtime error: uninitialized state, invariant violation, etc.
    RuntimeError(String),

    // === Verifier (L7) ===
    /// Precondition check failed (verifier feature only).
    #[cfg(feature = "verifier")]
    PreconditionFailed(String),
    /// Postcondition check failed (verifier feature only).
    #[cfg(feature = "verifier")]
    PostconditionFailed(String),
    /// Invariant check failed (verifier feature only).
    #[cfg(feature = "verifier")]
    InvariantViolated(String),
    /// Proof not found for assertion (verifier feature only).
    #[cfg(feature = "verifier")]
    ProofNotFound(String),
    /// User assertion failed (verifier feature only).
    #[cfg(feature = "verifier")]
    AssertionFailed(String),
    /// Audit chain integrity error (verifier feature only).
    #[cfg(feature = "verifier")]
...
impl Runtime {
    ...
    fn dispatch_extract_placeholder(&mut self, target: &str, path: &str) -> Result<(), CnfError> {
        let buf = self.get_buffer(target)?;
        if !path.starts_with("$.") {
            return Err(CnfError::InvalidInstruction(path.to_string()));
        }
        let _text = String::from_utf8(buf.to_vec())
            .map_err(|_| CnfError::InvalidInstruction("non-utf8 buffer".into()))?;
        Ok(())
    }
}

// === TypeValidator trait and implementations (module scope, OUTSIDE impl Runtime) ===
...
// ==================
// End of TypeValidator trait and impl blocks
// ==================

impl Runtime {
    /// Execute EXTRACT instruction (placeholder).
    ///
    /// Only JSON is supported at the moment.  The path must start with
    /// `$.` and subsequent identifiers are treated as object keys.  The
    /// extracted value is serialized to string and stored in a new buffer
